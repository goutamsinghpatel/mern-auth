...
export const sendVerifyOtp=async(req,res)=>{
  ...
  try{
   ...
   if(user.isAcountVerified){
    return res.json({succes:false,message:"account Alredy   verified"});
   }
...
console.log(err.message)
 ...
 return res.json({succes:true,message:"Verification Otp send on email"}); 
...
export const verify=async(req,res)=>{
     ...
     const {userId}=req;
     if(!userId || !Otp){
      return res.json({succes:false,message:'missing detailes'})
     }
 try {
...
await user.save();
return res.json({succes:true,message:" Email verify successfully"});
...
export  const isAuthentication=async(req,res)=>{
  try {
    return res.json({succes:true,message:"you are authenticated"});
...
console.log(err.message)
 ...
 return res.json({succes:true,message:"reset  Otp send on email"}); 
...
export const resetPassword=async(req,res)=>{
  ...
  try {
    ...
    if(user.resetOtp==='' || user.resetOtp!==Otp){
      return res.json({succes:false,message:"invailid Otp"});
    }
    ...
    await user.save();
    return res.json({succes:true,message:"password has been succesfully reset"});
